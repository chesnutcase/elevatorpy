    ...
    async def step(self, system):
        while system.count_total_passengers() > 0 or len(self.passengers) != 0:
            ...
            if len(self.passengers) != 0:
                ...
                try:
                    closest_destination = min([floor for (floor, passengers) in system.floors.items() if len(passengers) != 0], key=(lambda d: abs(self.floor - d)))
                except ValueError:
                    closest_destination = self.home_floor
...
class NaiveElevatorSystem(ElevatorSystem):
    def __init__(self, name, *, num_floors=5, num_lifts=3, stream=None):
        ...
        for i in range(num_lifts):
            new_elevator = NaiveElevator(name="NaiveElevator {}".format(i), stream=stream)
